...
// 1000 packets × 26 bytes = 26 KB.
// Duration = RING_CAP × decimate / 80 SPS  (e.g. decimate=10 → ~125 s at 8 Hz)
...
static volatile int        s_decimate   = 1;   // store 1-of-every-N samples
static int                 s_dec_ctr    = 0;   // counts samples since last stored (tx_task only)
...
static void ring_push(const ts_espnow_packet_t *pkt)
{
    ...
    if (++s_dec_ctr < s_decimate) return;   // decimation: skip N-1 out of every N
    s_dec_ctr = 0;
...
static esp_err_t status_handler(httpd_req_t *req)
{
    ...
    snprintf(json, sizeof(json),
             "{\"count\":%d,\"cap\":%d,\"collecting\":%s,\"decimate\":%d,\"sps\":80}",
             count, RING_CAP, s_collecting ? "true" : "false", s_decimate);
...
// ─── Calibration profiles (NVS) ──────────────────────────────────────────────
// Namespace: "thruststand"  Keys: "cal00" .. "cal99"
// Each blob is a JSON object: {"name":...,"c0":{offset,scale,unit,maxN},"c1":{...}}
// The NVS key is NOT stored inside the blob; it is injected on read.
#define CAL_NVS_NS  "thruststand"
#define CAL_KEY_FMT "cal%02d"
#define CAL_SLOTS   100

// ─── Minimal JSON helpers (no external library) ───────────────────────────────
// Extract the string value of a key from a flat JSON object string.
// e.g. json_get_str("{\"name\":\"foo\"}", "name", buf, 32) → buf = "foo"
...
// GET /cal — returns JSON array of all saved profiles (full blob + "key" field injected)
static esp_err_t cal_get_handler(httpd_req_t *req)
{
    ...
    if (nvs_open(CAL_NVS_NS, NVS_READONLY, &h) == ESP_OK) {
        ...
        for (int i = 0; i < CAL_SLOTS; i++) {
            snprintf(key, sizeof(key), CAL_KEY_FMT, i);
            size_t sz = sizeof(blob) - 1;
            if (nvs_get_blob(h, key, blob, &sz) == ESP_OK) {
                blob[sz] = '\0';
                // Strip closing '}' and inject "key" field
                char *end = strrchr(blob, '}');
                if (!end) continue;
                *end = '\0';
                char entry[600];
                snprintf(entry, sizeof(entry), "%s%s,\"key\":\"%s\"}",
                         first ? "" : ",", blob, key);
                first = false;
...
// POST /cal — body: JSON with "key" field (cal00-cal99); key stripped before storing
static esp_err_t cal_post_handler(httpd_req_t *req)
{
    if (req->content_len <= 0 || req->content_len > 511) {
    ...
    char slot[8] = {0};
    if (!json_get_str(body, "key", slot, sizeof(slot))) {
        httpd_resp_send_err(req, HTTPD_400_BAD_REQUEST, "missing key");
        return ESP_FAIL;
    }

    // Remove the "key" field from the blob before storing
    // Find ,"key":"calXX" or "key":"calXX", and remove it
    char to_store[512];
    strncpy(to_store, body, sizeof(to_store) - 1);
    to_store[sizeof(to_store) - 1] = '\0';
    char key_fragment[24];
    snprintf(key_fragment, sizeof(key_fragment), ",\"key\":\"%s\"}", slot);
    char *kp = strstr(to_store, key_fragment);
    if (kp) {
        strcpy(kp, "}");
    } else {
        snprintf(key_fragment, sizeof(key_fragment), "\"key\":\"%s\",", slot);
        kp = strstr(to_store, key_fragment);
        if (kp) memmove(kp, kp + strlen(key_fragment), strlen(kp + strlen(key_fragment)) + 1);
    }

    nvs_handle_t h;
    esp_err_t err = nvs_open(CAL_NVS_NS, NVS_READWRITE, &h);
    if (err == ESP_OK) {
        err = nvs_set_blob(h, slot, to_store, strlen(to_store));
...
// DELETE /cal?key=cal00
static esp_err_t cal_delete_handler(httpd_req_t *req)
{
    char key[16] = {0};
    ...
    if (qlen > 0) {
        ...
        httpd_query_key_value(qbuf, "key", key, sizeof(key));
    }
    if (!key[0]) {
        httpd_resp_send_err(req, HTTPD_400_BAD_REQUEST, "missing key");
        return ESP_FAIL;
...
// ─── HTTP: POST /config — body: {"decimate":N} ───────────────────────────────
static esp_err_t config_post_handler(httpd_req_t *req)
{
    ...
    const char *p = strstr(body, "\"decimate\":");
    if (p) {
        p += 11;
        while (*p == ' ') p++;
        int d = atoi(p);
...
static const httpd_uri_t s_uri_cal_get = {
    .uri     = "/cal",
    .method  = HTTP_GET,
    .handler = cal_get_handler,
};

static const httpd_uri_t s_uri_cal_post = {
    .uri     = "/cal",
    .method  = HTTP_POST,
    .handler = cal_post_handler,
};

static const httpd_uri_t s_uri_cal_delete = {
    .uri     = "/caldel",
    .method  = HTTP_GET,
    .handler = cal_delete_handler,
};
...
static void start_http_server(void)
{
    ...
    httpd_register_uri_handler(s_server, &s_uri_cal_get);
    httpd_register_uri_handler(s_server, &s_uri_cal_post);
    httpd_register_uri_handler(s_server, &s_uri_cal_delete);
